...
def main():
    ...
    shortest = shortest = bfs_find_dist(elevations, start, end)
    for row in range(0, len(elevations)):
        for col in range(0, len(elevations[0])):
            if (elevations[row][col] == 0):
                possible_shortest = bfs_find_dist(elevations, (row, col), end)

                if possible_shortest < shortest:
